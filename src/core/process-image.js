...
export function resize(options) {
  let { m = 'lfit', w,h,l,s,limit = 1,color = 'ffffff', p = 100 } = options
  if (p > 100) limit = 0
 ...
 * @returns {string} 返回处理query
 */
export function roundedCorners(r = 1) {
 ...
 * @returns {string} 返回处理query
 */
export function autoOrient(value = 1) {
 ...
 * @returns {string} 返回处理query
 */
export function sharpen(value = 100) {
 ...
 * @returns {string} 返回处理query
 */
export function format(type = 'webp') {
  return `/format,${type}`
}
 ...
 * @returns {string} 返回处理query
 */
export function quality(value, type = 'Q') {
...
export function watermark(options) {
  let { t = 100, g = 'se', x = 10, y = 10, voffset = 0, text = '', color = '000000', size = 40, rotate = 0 } = options
